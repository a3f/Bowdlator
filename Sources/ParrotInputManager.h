...
*/
#define InputManager [ParrotInputManager sharedManager]
/*!
...
*/
@interface ParrotInputManager : NSObject<IMKServerInputTextData> {
@private
    IMKServer *server;
...
/*!
    ...
    하지만 구현은 외부 환경에 의존적이기 때문에 미리 되어있지는 않다. CharmIM에서는 @ref ParrotAppDelegate 에 생성한 객체를 공유객체로 사용하도록 구현되어 있다.
*/
@interface ParrotInputManager (SharedObject)
